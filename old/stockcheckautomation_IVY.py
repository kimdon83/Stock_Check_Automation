...
scp_dir= r'C:\OneDrive\OneDrive - Kiss Products Inc\Desktop\stock check practice'
...
order_df = pd.read_csv(scp_dir+'\\Stock Check Request.csv')
...
if len(order_df) > 0:
    ...
    subprocess.check_call(['C:\Program Files (x86)\SAP\FrontEnd\SAPgui\\sapshcut.exe', '-system=P01', '-client=100', '-user=IVY_SADM_09', '-pw=wert2345', 
    #  '-command=va05', 
     '-command=ZPPRMRP01', 
...
#            session.findById("wnd[1]/usr/ctxtDY_PATH").text = "Y:\\OM ONLY_Shared Documents\\3 Teams\\2 DP\\38. Stockcheck" #Change Location - (type 1)
    ...
    final_df.to_excel(resultLoc+"\\"+order_number+"Result_IVY.xlsx") #Change Location - (type 2)
    ...
    wb.save(resultLoc+"\\"+order_number+"Result_IVY.xlsx") #Change Location - (type 2)
    wb.close()
