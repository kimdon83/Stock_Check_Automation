...
def salesorg(df):
    if df['Requester'] in ['Nancy Roh', 'Sandy Lee', 'Jay Jun']: # Could be revised if PIC changed
        return '1300'
    else:
        return '1100'
...
if len(order_df) > 0:
    ...
    subprocess.check_call(['C:\Program Files (x86)\SAP\FrontEnd\SAPgui\\sapshcut.exe', '-system=P01', '-client=100', '-user=IVY_SADM_05', '-pw=asd1234', 
    # '-command=va05', 
    '-command=ZPPRMRP01', 
    ...
    # %% Import Input file
    input_df = pd.read_excel(r'C:\Users\KISS Admin\Desktop\stock check practice\input.xlsx', sheet_name='Sheet1') #Change Location - (type 1)
    ...
    stock_df['requirements'] = stock_df['requirements'] * 53
    # %%
    # FCST
    ...
    # %%
    # Export output
    final_df.to_excel(r'C:\Users\KISS Admin\Desktop\stock check practice\stockcheckresult_IVY.xlsx') #Change Location - (type 2)

    # %% Excel file formatting
    wb = load_workbook(r'C:\Users\KISS Admin\Desktop\stock check practice\stockcheckresult_IVY.xlsx') #Change Location - (type 2)
    ws = wb.active
    max_row = ws.max_row
    ...
    wb.save(r'C:\Users\KISS Admin\Desktop\stock check practice\stockcheckresult_IVY.xlsx') #Change Location - (type 2)
    wb.close()
