...
if(len(inputKDC)>0):
    ...
    for index, row in df_total.iterrows():
        # print(row.On_hand_qty)
        # print(inputKDC[inputKDC['material']==row.mtrl].qty)
...
for index, row in df_result.iterrows():
    ...
    if row["availability"]=="NO":
        exp_bodate=datetime.strptime(row.eta.split('_')[0],'%Y-%m-%d')
